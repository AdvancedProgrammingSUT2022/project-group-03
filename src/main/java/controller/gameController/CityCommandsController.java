...
public class CityCommandsController {
    public static int reAssignCitizen(int originX,
        ...
        if (originX < 0 || originY < 0 ||
            ...
            return 1;
        if (GameController.getSelectedCity().assignCitizenToTiles(GameController.getMap().coordinatesToTile(originX, originY),
    ...
    public static int assignCitizen(int destinationX, int destinationY) {
        ...
        if (GameController.getSelectedCity().getCivilization() != GameController.getCivilizations().get(GameController.getPlayerTurn()))
            return 2;
        if (destinationY < 0 || destinationX < 0 || destinationX > GameController.getMap().getX() || destinationY > GameController.getMap().getY())
            return 1;
    ...
    public static int buyTile(int x, int y) {
        ...
        if (GameController.getSelectedCity() == null || GameController.getSelectedCity().getCivilization() != GameController.getCivilizations().get(GameController.getPlayerTurn())) return 4;
        if (!GameController.getSelectedCity().isTileNeighbor(GameController.getMap().coordinatesToTile(x, y))) return 3;
        if (GameController.getCivilizations().get(GameController.getPlayerTurn()).getGold() < 15 + 10 * (GameController.getSelectedCity().getTiles().size() - 6)) return 5;
        if (!GameController.getSelectedCity().addTile(GameController.getMap().coordinatesToTile(x, y))) return 1;
        GameController.getSelectedCity().getCivilization().changeGold(-(15 + 10 * (GameController.getSelectedCity().getTiles().size() - 6)));
        return 0;
    }
    ...
    public static int cityDestiny(boolean burn) {
        ...
        else GameController.getSelectedCity().changeCivilization(GameController.getCivilizations().get(GameController.getPlayerTurn()));
        GameController.deleteFromUnfinishedTasks(new Tasks(GameController.getSelectedCity().getMainTile(), TaskTypes.CITY_DESTINY));
        return 0;
    }
    ...
    public static int buyUnit(String string, int x, int y) {
        ...
        if (GameController.getCivilizations().get(GameController.getPlayerTurn()).getGold() < unitType.getCost())
            return 3;
        if ((unitType.combatType == CombatType.CIVILIAN && GameController.getMap().coordinatesToTile(x, y).getCivilian() != null) ||
                (unitType.combatType != CombatType.CIVILIAN && GameController.getMap().coordinatesToTile(x, y).getNonCivilian() != null))
            return 4;
