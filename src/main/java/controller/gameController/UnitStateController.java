...
public class UnitStateController {
    public static boolean unitMoveTo(int x, int y) {
        if (GameController.getSelectedUnit() == null || x < 0 || y < 0 ||
                x >= GameController.getMap().getX()||y > GameController.getMap().getY()
                 || GameController.getCivilizations().get(GameController.getPlayerTurn()) != GameController.getSelectedUnit().getCivilization()
            ...
            return false;
        GameController.deleteFromUnfinishedTasks(new Tasks(GameController.getSelectedUnit().getCurrentTile(), TaskTypes.UNIT));
        GameController.getSelectedUnit().setState(UnitState.AWAKE);
        return GameController.getSelectedUnit().move(GameController.getMap().coordinatesToTile(x, y), true);
    }
    ...
    public static int unitAlert() {
        ...
        if (GameController.openNewArea(GameController.getSelectedUnit().getCurrentTile(),
                GameController.getCivilizations().get(GameController.getPlayerTurn()), GameController.getSelectedUnit()))
            return 3;
        return 0;
    }
    ...
    public static int unitFoundCity(String string) {
        ...
        for (Civilization civilization : GameController.getCivilizations())
            if (civilization.isInTheCivilizationsBorder(GameController.getSelectedUnit().getCurrentTile()))
                return 4;
        for (Civilization civilization : GameController.getCivilizations())
        ...
        GameController.openNewArea(GameController.getSelectedUnit().getCurrentTile(), GameController.getCivilizations().get(GameController.getPlayerTurn()), null);
    ...
    public static int unitDelete(Unit unit) {
        if (unit == null)
            return 1;
        if (unit.getCivilization() != GameController.getCivilizations().get(GameController.getPlayerTurn()))
            return 2;
    ...
    public static int unitBuild(ImprovementType improvementType) {
        ...
        if (!GameController.doesHaveTheRequiredTechnologyToBuildImprovement(improvementType,
                GameController.getSelectedUnit().getCurrentTile(), GameController.getSelectedUnit().getCivilization()))
            return 4;
        if (!GameController.canHaveTheImprovement(GameController.getSelectedUnit().getCurrentTile(), improvementType))
            return 5;
        GameController.getSelectedUnit().getCurrentTile().setImprovement
                (new Improvement(improvementType, GameController.getSelectedUnit().getCurrentTile()));
    ...
    public static int unitBuildRoad() {
        ...
        GameController.getSelectedUnit().getCurrentTile().setRoad
                (new Improvement(ImprovementType.ROAD, GameController.getSelectedUnit().getCurrentTile()));
        GameController.getSelectedUnit().setState(UnitState.BUILDING);
        return 0;
    }
    ...
    public static int unitBuildRailRoad() {
        ...
        if (GameController.getSelectedUnit().getUnitType() != UnitType.WORKER)
            return 3;
        if (GameController.getCivilizations().get(GameController.getPlayerTurn()).doesContainTechnology(TechnologyType.RAILROAD) != 1)
            return 4;
        if (GameController.getSelectedUnit().getCurrentTile().getRoad() != null)
            return 6;
        GameController.deleteFromUnfinishedTasks(new Tasks(GameController.getSelectedUnit().getCurrentTile(),
                TaskTypes.UNIT));
        GameController.getSelectedUnit().getCurrentTile().setRoad
    ...
    public static int unitRemoveFromTile(boolean isJungle) {
        ...
        if (GameController.getSelectedUnit().getUnitType() != UnitType.WORKER)
            return 3;
        if (isJungle && (GameController.getSelectedUnit().getCurrentTile().getContainedFeature()== null ||
                (GameController.getSelectedUnit().getCurrentTile().getContainedFeature() != null &&
                GameController.getSelectedUnit().getCurrentTile().getContainedFeature().getFeatureType() != FeatureType.JUNGLE &&
                GameController.getSelectedUnit().getCurrentTile().getContainedFeature().getFeatureType() != FeatureType.FOREST)))
            return 4;
    ...
    public static int unitAttack(int x, int y) {
        ...
        if(GameController.getSelectedUnit().getUnitType().combatType == CombatType.SIEGE &&
                (GameController.getSelectedUnit().getState() != UnitState.SETUP ||  ((NonCivilian) GameController.getSelectedUnit()).getFortifiedCycle() < 1))
            return 7;
        ...
        if (!GameController.getSelectedUnit().move(GameController.getMap().coordinatesToTile(x, y), true)) return 6;
    ...
    public static int skipUnitTask() {
        ...
        if (GameController.getSelectedUnit().getCivilization() != GameController.getCivilizations().get(GameController.getPlayerTurn()))
            return 2;
        if (GameController.findTask(new Tasks(GameController.getSelectedUnit().getCurrentTile(), TaskTypes.UNIT)) == null)
            return 3;
        GameController.deleteFromUnfinishedTasks(new Tasks(GameController.getSelectedUnit().getCurrentTile(), TaskTypes.UNIT));
        return 0;
    }
